...
__global__ void _cuda_compute_correlation(double* correlation, double* vector_len1, double* vector_len2);
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < number_bacteria - 1; i++)
    for (int j = i + 1; j < number_bacteria; j++) {
      ...
      _cuda_compute_correlation<<<1, 1>>>(d_correlation + pos, bacteria[i]->vector_len, bacteria[j]->vector_len);
        pos++;
      }
...
void ProcessBacteria(Bacteria* b) {
  ...
  _cuda_stochastic_precompute<<<sm_count, thread_count>>>(M, M1, d_vector, d_second, d_one_l, b->total, 
      b->complement, b->total_l, b->dense_stochastic);
