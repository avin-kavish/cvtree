...
int main(int argc, char *argv[]) {
  ...
  std::cout << prop.name << "\tSM Count: " << prop.multiProcessorCount
            << "\t Max threads: " << prop.maxThreadsPerBlock << std::endl;
  ...
  for (int fi = 0; fi < number_bacteria;) {
    if (current_loads < MAX_CONCURRENT_LOADS) {
      ...
      std::cout << "Loaded " << fi + 1 << " of " << number_bacteria
                << std::endl;

      current_loads++;
      threads[fi] =
          std::async(std::launch::async, ProcessBacteria, bacteria[fi]);
      fi++;
    }
  }
...
void ProcessBacteria(Bacteria *b) {
  ...
  while (cudaStreamQuery(stream) != 0)
...
void MultiThreadedCPUCompare(Bacteria **bacteria) {
  ...
  for (int i = 0; i < number_bacteria - 1; i++)
    for (int j = i + 1; j < number_bacteria; j++) {
      ...
      compare_threads.push_back(
          std::async(std::launch::async, CompareBacteria, c));
    }
